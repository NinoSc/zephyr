...
LOG_MODULE_REGISTER(video_pio_dma, /*CONFIG_VIDEO_LOG_LEVEL*/ LOG_LEVEL_DBG);
				       ...
				       int status)
{
	...
	if (status >= 0) {
		atomic_set_bit(&data->stream_evt, VIDEO_PIO_DMA_RX_BIT);
		...
		if (data->dma_channel >= 0) {
			...
			if (ret == 0) {
				...
				if (ret == 0) {
					...
					pio_sm_set_enabled(data->pio, data->pio_sm, true);
					pio_sm_put_blocking(data->pio, data->pio_sm,
							    data->fmt.width * data->pclk_per_px -
								    1);
				} else {
					LOG_ERR("Failed to start DMA (%d)", ret);
				}
			...
			/* Return the error code from dma_request_channel */
			ret = data->dma_channel;
		}
	} else {
...
static int video_pio_dma_set_fmt(const struct device *dev, struct video_format *fmt)
{
	const struct video_pio_dma_config *config = dev->config;
	struct video_pio_dma_data *data = dev->data;
	int ret = video_set_format(config->sensor_dev, fmt);
...
static int video_pio_dma_set_ctrl(const struct device *dev, uint32_t id)
{
	return 0;
}
...
static void video_pio_dma_worker(struct k_work *work)
{
	...
	struct video_pio_dma_data *data;
	int ret = 0;

	data = CONTAINER_OF(dwork, struct video_pio_dma_data, work);
...
static DEVICE_API(video, video_pio_dma_driver_api) = {.set_format = video_pio_dma_set_fmt,
						      ...
						      .set_ctrl = video_pio_dma_set_ctrl,
